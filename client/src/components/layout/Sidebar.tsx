...
export function Sidebar({ expanded = false, onToggle }: SidebarProps) {
  ...
  return (
    <nav className={`hidden md:flex flex-col h-screen fixed left-0 top-0 bg-[var(--color-surface-container-low)] border-r border-[var(--color-outline-variant)] z-40 transition-[width] duration-300 ease-in-out ${expanded ? 'w-60' : 'w-14'}`}>
      ...
      >
        {expanded ? (
           <>
             ...
             <span className="material-symbols-outlined text-[20px] text-[var(--color-outline)] opacity-0 group-hover:opacity-100 transition-opacity">
               menu_open
             </span>
           </>
        ) : (
           <>
             <span className="font-mono text-[16px] font-bold text-[var(--color-primary)] tracking-tight group-hover:opacity-0 transition-opacity absolute">
               E
             </span>
             <span className="material-symbols-outlined text-[20px] text-[var(--color-outline)] opacity-0 group-hover:opacity-100 transition-opacity absolute">
               menu
             </span>
           </>
        )}
              ...
              {group.items.map(({ to, icon, label }) => (
                ...
                >
                  {({ isActive }) => (
                    <>
                      {!expanded && isActive && (
                        <div className="absolute left-0 top-[20%] bottom-[20%] w-[2px] bg-[var(--color-primary)]" />
                      )}
                      <span
                      ...
                      {!expanded && <span className="tooltip-text">{label}</span>}
                    </>
                  )}
                </NavLink>
              ))}
      ...
      <div className={`flex border-t border-[var(--color-outline-variant)] py-2 ${expanded ? 'items-center px-4 justify-between flex-row' : 'flex-col items-center gap-0.5 px-2'}`}>
        ...
        >
          ...
          {expanded && <span className="font-mono text-[11px] uppercase tracking-wider ml-2 whitespace-nowrap">Theme</span>}
          {!expanded && <span className="tooltip-text">{theme === 'dark' ? 'Light Mode' : 'Dark Mode'}</span>}
        </button>
        ...
        >
          <div className="w-7 h-7 flex items-center justify-center font-mono text-[10px] font-bold border border-[var(--color-outline-variant)] bg-[var(--color-surface-container-high)] text-[var(--color-on-surface-variant)] shrink-0">
          ...
          {!expanded && <span className="tooltip-text">Settings</span>}
        </NavLink>
      </div>
    </nav>
  );
